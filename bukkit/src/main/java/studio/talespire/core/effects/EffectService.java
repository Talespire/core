...
import studio.talespire.core.effects.commands.EffectCommand;
import studio.talespire.core.effects.util.*;
...
@Getter
@Setter
public class EffectService implements Disposable {
    private static final List<EffectService> EFFECT_SERVICES = new ArrayList<>();
    ...
    public EffectService(Plugin owningPlugin) {
        this(owningPlugin, owningPlugin.getLogger());
    }

    public EffectService(Plugin owningPlugin, Logger logger) {
        ...
        EFFECT_SERVICES.add(this);

        CommandUtil.registerAll(new EffectCommand());
    }

    public static List<EffectService> getManagers() {
    ...
    public Effect getEffectByClassName(String effectClass) {
        ...
        Effect effect = null;
        try {
            Constructor<? extends Effect> constructor = effectLibClass.getConstructor(EffectService.class);
    ...
    protected boolean setField(Object effect, String key, ConfigurationSection section, ConfigurationSection parameterMap, String logContext) {
        try {
            ...
            } else if (field.getType().equals(org.bukkit.util.Vector.class)) {
                String value = fieldSection.getString(fieldKey);
                if (value != null) {
    ...
    public static void disposeAll() {
        Iterator<EffectService> i = EFFECT_SERVICES.iterator();
        while (i.hasNext()) {
            EffectService em = i.next();
            i.remove();
            em.dispose();
        }
    }
